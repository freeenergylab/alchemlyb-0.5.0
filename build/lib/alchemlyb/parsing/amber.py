        ...
        for line in secp:
            ...
            if line.startswith(' NSTEP'):
                if in_comps:
                    ...
                    nstep, restraint, dvdl = secp.extract_section(
                        '^ NSTEP', '^ ---', ['NSTEP', 'RESTRAINT', 'DV/DL'],
                        extra=line
                        )
                    # If dvdl always is 0.0, AMBER20 won't print it.
                    # Here, when dvdl is None and RESTRAINT is not None,
                    # we think it belongs to this situation
                    if dvdl is None and restraint is not None:
                        dvdl = 0.0
